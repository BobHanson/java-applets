...
// Es werden Datensätze (kurz: DSS) zufällig generiert, dies sind zum einen 
// DSS aus [0 - 9999] und DSS, die Geburtsdaten [TTMM] darstellen. 
// Ausserdem können Datensätze [0 - 9999] einzeln eingegeben werden.
// Hierzu werden Hash-Codes aus der "Algorithmen & DS" -Vorlesung verwendet.
// Hierzu zählen: 4 Divisionsmethoden und 1 Multiplikationsmethode.
// 
...
public class Hash extends Applet
	...
	*/
    ...
    // Attribute und Objekte für die einzelnen Klassenmethoden
    ...
    // Festlegung von Schriftfonts für die einzelnen Texte der Label

	/*
    private String text1 = " Hashing Methoden ";
    private String text2 = " Bitte die Anzahl an Datenfeldern auswählen!";
    ...
    */
    ...
    // Überlagert wird die Methode init(), die das Applet initialisiert
    public void init() {
	  ...
	  // Die Choice für die Feldanzahl wird gesetzt.
	  NeueTabelle();
	  // Die Choice für die Methoden wird gesetzt.
	  NeueHashWahl();
	...
	// Panel ebene0 enthält ein zusätzliches (z. T. leeres) Label
	ebene0 = new Panel();
	...
	// ebene1 enthält die Choice für Datenfelder und Hash-Methoden.
	...
	methodeChoice.setBackground( Color.white );
	methodeChoice.addItemListener( this );
	// Die Choices werden dem panel im FlowLayout hinzugefügt.
	ebene1.add( groesseChoice );
	ebene1.add( methodeChoice );


	// Das Panel ebene2 soll alle Buttons für DSS-eingabe enthalten,
	// sowie deren Textfelder. ebene2 enthält Button Zufall-1 mit TF,
	...
	// ebene3 enthält Buttons "Start Hashing" und "Neues Hashing".
	// ebene3 enthält daher die Buttons k6 und k7.
    ...
    */
	...
	// dabei soll Hashfilm.java später noch zu 'Center' hinzugefügt
	...
	// Gleiches gilt für Panel mitte, das die Klasse HashFilm verwaltet.
	...
	// benötigten Textfelder. Button "Start Hashing" führt mit Eingabe-
...
fazit.setText("");
	...
	// labeloben.setText("DSS  =  Datensatzschlüssel");


	if (cmd.equals("einf\u00fcgen"))
	{
	    // Hauptprogramm: Ermittlung und Übergabe der jew. Hashparameter.
	    // Überprüft 1 editierbares Textfeld (andere sind blockiert).
	    ...
	    // Falls Objekt bereits existiert, wird dies benutzt (global!).
		    ...
		    {
			if (count == 1)
			    fazit.setText(count+" zufälliges Geburtsdatum [TTMM] wird eingef\u00fcgt");
			else
			    fazit.setText(count+" zufällige Geburtsdaten [TTMM] werden eingef\u00fcgt");
			BerechneZufall2();
			LabelAngabe();
		    } // Ende tf4.isEditable
	...
	if (cmd.equals("neu"))
	{
	    ...
	    // labeloben.setText("Erneute Eingaben für Hashing-Methoden");
	    // leereslabel.setText("");
	    LabelAngabe();
...
/*
	...
	if (cmd.equals("DSS-Eingabe"))
	{
	    ...
	    //tf3.addTextListener( this );
	    tf3.setText("");
	    // Bei DSS-Eingabe wird Zufallsauswahl komplett hinfällig.
	    tf2.setText("0");
	    tf2.setEditable(false);
	    tf4.setText("0");
	    tf4.setEditable(false);
	...
	if (cmd.equals("einf\u00fcgen"))
	{
	    ...
	    // Falls Objekt bereits existiert, wird dies benutzt (global!).
		    ...
		    {
			if (count == 1)
			    fazit.setText(count+" zufälliger Datensatz aus: [0 - 9999] wird erzeugt!");
			else
			    fazit.setText(count+" zufällige Datensätze aus: [0 - 9999] werden erzeugt!");
			BerechneZufall1();
			LabelAngabe();
		    } // Ende tf2.isEditable()
		    ...
		    {
			if (count == 1)
			    fazit.setText(count+" zufälliges Geburtsdatum [TTMM] wird erzeugt!");
			else
			    fazit.setText(count+" zufällige Geburtsdaten [TTMM] werden erzeugt!");
			BerechneZufall2();
			LabelAngabe();
		    } // Ende tf4.isEditable
		...
		// Eingabeparameter falsch.
		else fazit.setText("Bitte Eingaben überprüfen und ggf. korrigieren!");
	    } // if-Ende für den Fall, daß 'Zufall' (tf2, tf3) gewählt wurde.
		...
		if ((feldanzahl != 0) && (methode != 0) && (key != -1))
		{
		    ...
		    // Anschließend erfolgt Übergabe der Hashparameter.
		    ...
		    // Fülle den Eingabedatensatz mit "Nullen" auf, um
	    ...
	    } // if-Ende für den Fall, daß Einzelauswahl gewählt wurde.
...
*/
    ...
    // Implementiere die Methode für das Interface ItemListener.
    // Überschrieben wird die (abstr.) Methode des Interfaces.
	...
	if ((feldanzahl != 0) && (Hashing != null))
	{
		...
		} // äußere for-Anweisung
		// Alter Keywert soll noch verwendet werden können.
		key = alterkey;
	    } // Ende if-Anweisung: Durchführung bei vollst. Eingabe.
	} // Ende if-Anweisung für entspr. Umverteilung der Datensätze.


	// Setzen der entspr. Label, falls Benutzereingaben fehlen!

	if ((feldanzahl == 0) && (methode == 0))
	    fazit.setText("Zuerst Gr\u00fc\u00dfe und Funktion auswählen!");
	if ((feldanzahl != 0) && (methode == 0))
	    fazit.setText("Die Funktion noch auswählen!");
	if ((feldanzahl == 0) && (methode != 0))
	    fazit.setText("Die Gr\u00fc\u00dfe noch auswählen!");
	...
	if ((methode == 0) && (firstchoice))
	    fazit.setText("Bitte eine Hash-Funktion auswählen!");
	...
	{
	    // Ein Datensatz (allgemein) wird zufällig ermittelt.
	...
	for (int i = 1; i <= count; i++)
	{
	    // Ein Geburtsdatum wird zufällig ermittelt.
	    // Anschließend erfolgt der Datentransfer.
	...
	// Ermittelt wird der Hashwert zu dem Datenschlüssel (key).
	// Es erfolgt eine Aufdatierung der jew. Tabellensäule,
	// sowie das Einzeichnen des entspr. Säulenstücks.
    ...
    private void NeueTabelle() { // Choice für die Feldanzahl
    ...
    private void NeueHashWahl() { // Choice für die Hashmethode.
